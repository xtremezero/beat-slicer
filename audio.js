...
async function analyzeAudioBeats(buffer) {
    ...
    initSeed(CONFIG.seed); // Ensure deterministic generation per run
    ...
    for (let i = 0; i < rawData.length; i += windowSize) {
        ...
        if (flux > CONFIG.fluxSensitivity && i - lastBeatSample >= minGapSamples) {
            const isDot = seededRandom() < 0.2;
            const pos = GRID_POSITIONS[Math.floor(seededRandom() * GRID_POSITIONS.length)];
            
            let dir = null;
            if (!isDot) {
                // Natural mapping: top row = UP, bottom row = DOWN. Left/Right edges = OUTWARD.
                if (pos.y < 0) {
                    dir = seededRandom() > 0.4 ? { angle: -90 } : (pos.x < 0 ? { angle: 180 } : (pos.x > 0 ? { angle: 0 } : { angle: -90 }));
                } else {
                    dir = seededRandom() > 0.4 ? { angle: 90 } : (pos.x < 0 ? { angle: 180 } : (pos.x > 0 ? { angle: 0 } : { angle: 90 }));
                }
            }
            ...
            beats.push({
                ...
                pos: pos,
                dir: dir,
                isDot, spawned: false, id: seededRandom().toString(36)
            });
            lastBeatSample = i;
        }
        prevEnergy = energy;
